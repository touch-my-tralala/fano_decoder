...
def code3_4():
    ...
    perforate_mask = '111010'  # for 'Intelsat 3/4'
    ...
    while True:
        ...
        if k >= num_iter:
            ...
            for i in range(sym_num):
                # Генерация ПСП -> кодирование -> перфорация ->
                prs_sym = prsGen.generate()
                prs_data += prs_sym
                encode_word = encoder.encode(prs_sym)
                perforate_word = encoder.perforate(encode_word)
                if perforate_word:
                    encode_seq += perforate_word
                    # -> Модулирование -> + абгш -> демодулирование ->
                    demode_word = ''
                    for j in range(len(perforate_word) // mod_order[mod_type]):
                        I, Q = modulator(perforate_word[mod_order[mod_type] * j:
                                                        mod_order[mod_type] * j + mod_order[mod_type]])
                        x, y = awgn_generate()
                        noisy_I = I + x * noise_sigma
                        noisy_Q = Q + y * noise_sigma
                        demode_word += demode.hard_decision(noisy_I, noisy_Q)
                        demode_buf += demode_word

                    # деперфорация -> Декодирование ->
                    deperforate_word = decoder.deperforator(demode_word)
                    for j in range(len(deperforate_word) // 2):
                        decode_seq += decoder.decode(deperforate_word[2 * j:2 * j + 2], cur_back_step,
                                                     perforate_mask[2 * j:2 * j + 2])
